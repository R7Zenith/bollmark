        ...
        <form
          ...
          onSubmit={(e) => {
            if (productCount > 0) {
              e.preventDefault();
              window.alert(
                `Bu markaya bağlı ${productCount} ürün var. Önce bu ürünleri başka bir markaya taşıyın veya markasız bırakın.`
              );
              return;
            }
            if (!window.confirm(`"${name}" markasını silmek istediğinize emin misiniz?`)) {
              e.preventDefault();
            }
          }}
        >
