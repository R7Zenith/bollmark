...
- **Arastirma bulgulari**: slazenger.com.tr'nin `/autocomplete/?search_text=` uc noktasi gercekten 404 donuyor (plandaki supheyi dogruladi) - ama urun sayfalarinin `?format=json`i Koton ile birebir ayni sema kullaniyor. `.env`deki `GOOGLE_CSE_API_KEY` gecersiz cikti ("API Key not found" hatasi) - bu **onceden var olan, bu degisiklikle ilgisiz bir yapilandirma sorunu** (Koton icin de calismiyordu), Google CSE yedek arama bu yuzden hicbir markada aktif degil; duzeltmek icin gecerli bir anahtar `.env`e girilmeli.
...
- **Commit/push**: Kullanici "pushla" dedi - commit atilip origin'e push edildi.
- **Commit/push**: Yerel commit atildi, push EDILMEDI.
