...
4. **Header alti 1-2px bosluk**: Kok nedeni (paylasilan `site-header.tsx`/`layout.tsx` spacer'i) tum sitede degistirmek risk oldugu icin, plandaki Secenek B (yerel/sadece /odeme) uygulandi: `checkout-form.tsx`'in en ust wrapper div'ine `-mt-px` eklendi. Not: bu oturumda makinede Python/Playwright kurulu olmadigindan bosluğun diger sayfalarda da (fark edilmeden) var olup olmadigi gorsel olarak olculemedi - sadece `npx tsc --noEmit` temiz ve dev server'da `/odeme` 200 donup derleme hatasiz calisti diye dogrulandi. Kullanici gorsel olarak yetersiz/fazla bulursa `-mt-[2px]` gibi bir deger denenebilir.
- **Dogrulama**: `npx tsc --noEmit` temiz. Dev server (`npm run dev`, zaten acikti) yeniden derledi, `/odeme` GET 200, konsolda hata yok.
