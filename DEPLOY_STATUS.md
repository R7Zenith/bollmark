...
- `checkout-form.tsx`'teki `summaryPanel`e ("Toplam" satirindan sonra) yeni bir blok eklendi: ayni `/payment/iyzico-logo-band-white.svg` (footer'daki ile ayni dosya, sadece o dosya beyaz zeminde degil beyaz LOGO oldugu icin) `bg-ink` renkli, `rounded-full` bir "hap" icinde gosteriliyor - aksi halde beyaz logo acik gri panelin (`bg-ink/5`) uzerinde gorunmezdi. Mobil/masaustu ayni `<details>` agacinin parcasi oldugu icin ekstra kod gerekmedi.
